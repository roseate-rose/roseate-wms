...
- Result: Operators can now enter the order-import flow by selecting a channel rather than typing one, WeChat Shop is the first/default option, and a CSV sample can be downloaded directly before upload. `npm --prefix frontend run build` passed after the UX changes.

## 2026-03-26 Import Alignment + Item Master CRUD + WeChat Template + Visible Import Guidance
- Why: The requested operational fixes clustered around one data-integrity theme: product import misalignment was polluting master data, admins had no direct way to repair those records in the UI, WeChat 小店 had become the highest-volume order source without a dedicated parser, and several inventory-facing screens rendered bare numbers without unit context, which made `0.2 盒`-style business interpretation difficult. The operator workflow also needed much more visible schema guidance so staff could validate templates before committing imports.
- How: Hardened `backend/services/tabular_service.py` to sanitize CSV/Excel headers, ignore empty `Unnamed:*` columns, and preserve non-empty unnamed columns without shifting row data. Updated `backend/services/product_import_service.py` to resolve explicit product-field aliases (including common Chinese headers) instead of depending on exact raw column names. Added admin-only `PUT /api/v1/products/<hb_code>` and `DELETE /api/v1/products/<hb_code>` in `backend/app.py`, with delete protection when inventory/order/receipt dependencies exist. Extended `backend/services/order_import_service.py` with `template=wechat_shop` header matching based on the provided CSV sample, mapping `订单号` / `SKU编码(自定义)` / `商品编码(自定义)` / `商品数量` while preserving the remaining export fields in `extra_data.row_extra`. Extended `frontend/src/views/ProductsView.vue` to support inline item-master editing/deletion plus a clearly visible product-import header reference and recognition cards. Extended `frontend/src/views/OrdersImportView.vue` with template docs, required/optional mapping status cards, and the visible WeChat header list. Added unit metadata to serialized orders/batches and introduced `frontend/src/utils/quantity.js`, then updated products/detail/inbound/orders/stock views to render unit-suffixed quantities with derived decimal purchase-unit displays.
- Result: `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest backend/tests` passes (38 tests), targeted import/order regression tests pass, and `npm --prefix frontend run build` passes. README, task records, and webtest handoff notes now reflect the implemented WeChat template support and the more explicit import UX.

## 2026-03-26 WebTest Handoff Contract
- Why: The main app agent and the separate webtest agent need a stable, machine-readable handoff so verification can be triggered without touching the webtest repo directly or relying on manual copy/paste.
- How: Added `tasks/webtest-handoff.json` as the producer-side contract in `roseate-wms`. The file defines a small status model (`triaged`, `implemented`, `ready_for_webtest`, `webtest_passed`, `webtest_failed`), a polling contract for the downstream consumer, and a `requests` array. Seeded the file with the current BUG-09 verification request, including changed files, expected behavior, local verification, and the suggested Playwright spec.
- Result: The separate webtest agent can now poll a single JSON file in the main repo and trigger only the relevant browser checks when a request status is `ready_for_webtest`.

## 2026-03-26 BUG-09 Settings Route RBAC Fix
- Why: The webtest feedback had drifted from the main repo tracker. The still-open item was a frontend RBAC gap: `staff` could reach `/settings` and still saw the "设置" sidebar link, even though the admin-only content inside the page was partially hidden.
- How: Updated `frontend/src/router/index.js` so `/settings` uses `meta.adminOnly`, which lets the existing global `beforeEach` guard redirect non-admin users back to `/`. Updated `frontend/src/layouts/MainLayout.vue` so nav items can be marked `adminOnly` and filtered by the stored role before rendering, hiding the "设置" entry for non-admin users.
- Result: `npm --prefix frontend run build` passed. The main repo tracker (`tasks/webtest-bugs.md`) now records BUG-09 as `implemented`, with browser-level re-check intentionally deferred to the separate webtest project per user instruction.
...
## 2026-03-19 Auth Policy Update: 7-Day JWT + Seeded Staff Accounts + Settings Logout
